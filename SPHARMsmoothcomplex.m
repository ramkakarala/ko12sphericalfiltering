function [coord_smooth, coeff]=SPHARMsmoothcomplex(coord,L,sigma)
%---------------------------------------------------------------------------------------
...
for l=1:L
    fprintf(1,'%d,',l);   % degree l SPHARM bases are recycled for all subject reducing computational time.
    if (rem(l,20)==0)
        fprintf(1,'\n');
    end;
    for i=1:n_subject
...
cerr = norm(imag(coord_smooth(:)));
if (cerr > eps)
    fprintf(1,'Magnitude of imaginary part in approximation is %f\n',cerr);
end;
coord_smooth=squeeze(reshape(real(coord_smooth),n_subject,3,n_vertex));

%coeff=[betax betay betaz];
coeff.x=betax;
coeff.y=betay;
coeff.z=betaz;
return;