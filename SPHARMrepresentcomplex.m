function SPHARMsurf=SPHARMrepresentcomplex(surf,fourier_coeff,k)
%--------------------------------------------------------------------------
% WARNING: this is non MNI-mesh format and works for arbitary mesh
%
%  SPHARMsurf=SPHARMrepresentcompex(surf,fourier_coeff,k)
...
% July 6, 2007 created 
% Jan  9, 2008 modified
% June 2010: written by Ram Kakarala
%----------------------------------------------------------------------------------------
...
coord_recon=[xcoord ycoord zcoord]';
cerr = norm(imag(coord_recon(:)));
if (cerr > eps)
    fprintf(1,'Magnitude of imaginary part in approximation is %f\n',cerr);
end;
coord_recon=squeeze(reshape(real(coord_recon),n_subject,3,n_vertex));
%temp=[xcoord ycoord zcoord];
%SPHARMsurf.vertices=squeeze(reshape(temp,n_vertex,3));
SPHARMsurf.vertices=coord_recon;
